import type { Default, Make, Opengraph } from './types'
...
export default class Seo {
  private base: Partial<Default.Props>
  private seo: Partial<Make.Props>
  ...
  private build() {
    ...
    if (robotsProps) {
      ...
      for (const key in robotsProps) {
        ...
        params.push(`${prop}:${robotsProps[key as keyof Make.Robots]}`)
      }
    ...
    if (opengraph) {
      ...
      if (opengraph.url || canonical) {
        elements.push({
          tag: 'meta',
          props: { property: 'og:url', content: opengraph.url || canonical },
        })
      }
    ...
    if (config.metatag && config.metatag.length) {
      const hasViewport = config.metatag.some((tag) => tag.name && tag.name === 'viewport')
  ...
  public default(config: Partial<Default.Props>) {
    this.base = config
  ...
  public make(props: Partial<Make.Props>) {
  ...
  public render() {
    ...
    return build.map((el, index) => {
      if (el.tag === 'meta') {
        return <meta key={`${el.props.name || 'meta'}:${index}`} {...el.props} />
      }
